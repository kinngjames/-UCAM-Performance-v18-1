...
|---|---|---|---|
| C1 · lesionado sin bienestar | ADV-EDGE J6; `NO DISPONIBLE`; 4 asistencias `LESIONADO`; sin bienestar; `rpeExpected=0` | `compliance=0`; `pending=1`; `status=INCOMPLETO`; señal `pending` con `0/0 RPE`; `loadCompleteness=NO_EXPOSURE` | Estadio C1: eliminar `compliance` y `streak`; `wellbeingExpected=0`; `pending=0`; sin señal/razón administrativa; `status=SIN DATOS` hasta que C6 separe ambos ejes. |
...
| C6 · VIGILAR + pendiente | ADV-EDGE J8; RPE `6,5`; `zRpe=2,33`; bienestar incompleto (`stress=null`) | señales `rpe-z` (watch) + `pending` (info); `pending=1`; `compliance=50`; estado final `INCOMPLETO`; razones conservan ambas señales | Estadio C6: `pending=1`; `recordCompleteness=PARTIAL`; `signals=[rpe-z]`; razones solo RPE; `status=VIGILAR`. |
